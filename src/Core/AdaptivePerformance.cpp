...
std::uint64_t ComputeVisualRevision(const Preset& preset,
    ...
    MixDouble(hash, preset.equation.bloomThreshold);
    MixDouble(hash, preset.equation.bloomSoftKnee);
    Mix(hash, static_cast<std::uint64_t>(preset.equation.bloomRadius));
    MixDouble(hash, preset.equation.edgeLightingStrength);
    MixDouble(hash, preset.equation.depthStrength);
    Mix(hash, preset.equation.stripeAverageEnabled ? 1ULL : 0ULL);
    MixDouble(hash, preset.equation.stripeDensity);
    MixDouble(hash, preset.equation.stripePhase);
    MixDouble(hash, preset.equation.stripeStrength);
    Mix(hash, static_cast<std::uint64_t>(preset.equation.stripeStartIteration));
